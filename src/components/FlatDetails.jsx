...
import { ArrowLeft, Phone, User, CreditCard, Calendar, Plus, Trash2 } from 'lucide-react';
import { flatGraph } from '../data/sampleData';
import Navbar from './Navbar';

const FlatDetails = ({ flats, payments, user, onLogout, onDeletePayment, userView = false }) => {
  const { flatNumber } = useParams();
  const targetFlatNumber = userView ? user.flatNumber : flatNumber;
  const flat = flats.find(f => f.flatNumber === targetFlatNumber);
  const flatPayments = payments.filter(p => p.flatNumber === targetFlatNumber);
  ...
  const connectedFlats = flatGraph[targetFlatNumber] || [];
  
  // Calculate group stats
  const getGroupStats = () => {
    const allFlatsInGroup = [targetFlatNumber, ...connectedFlats];
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              <div className="mb-6 md:mb-0">
                <h1 className="text-3xl font-bold text-gray-800 mb-2">
                  {userView ? 'My Payment History' : `Flat ${targetFlatNumber}`}
                </h1>
              ...
              {user.role === 'admin' && (
                <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                  <Link
                    to="/add-payment"
                    state={{ flatNumber: targetFlatNumber }}
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-200">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {connectedFlats.map(fNumber => (
                        <Link
                          key={fNumber}
                          to={user.role === 'admin' ? `/flat/${fNumber}` : '#'}
                          className={`px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm transition-colors ${
                            user.role === 'admin' ? 'hover:bg-blue-200' : 'cursor-default'
                          }`}
                        >
                          {fNumber}
                        </Link>
